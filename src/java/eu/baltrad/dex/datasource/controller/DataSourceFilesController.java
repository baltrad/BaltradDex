...
public class DataSourceFilesController {
    ...
    private static final String FC_ERROR_MESSAGE_KEY = "browsefiles.file_catalog_error";
    ...
    public String processSubmit(
            ...
            ModelMap model,
            @RequestParam(value = "ds_name", required = false) String dataSourceName,
    ...
    private int[] getPages(String dataSourceName) {
        ...
        int numPages = (int) Math.ceil((double) numEntries
                / BltFileManager.ENTRIES_PER_PAGE);
        if (numPages < 1) {
            numPages = 1;
        }
    ...
    public void nextPage(String dataSourceName) {
        int lastPage = (int) Math.ceil((double) fileManager
                .count(dataSourceName) / BltFileManager.ENTRIES_PER_PAGE);
        if (lastPage == 0) {
            ++lastPage;
        }
