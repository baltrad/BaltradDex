...
public class BltFileManager implements IBltFileManager {
    ...
    private final static Logger logger = LogManager
            .getLogger(BltFileManager.class);
     ...
     * @throws DatabaseError
     *             In case when file catalog is not available
     */
    ...
    private BltFile fileEntryToBltFile(FileEntry entry) throws DatabaseError {
        ...
        try {
            ...
            synchronized (format) {
                file = new BltFile(entry.getUuid().toString(), fileCatalog
                        .getLocalStorage().store(entry).toString(),
                        format.parse(metadata.getWhatDate().toIsoString() + "T"
                                + metadata.getWhatTime().toIsoString()),
                        format.parse(entry.getStoredDate().toIsoString() + "T"
                                + entry.getStoredTime().toIsoString()),
                        metadata.getWhatSource(), metadata.getWhatObject(),
    ...
    public List<String> loadDistinctRadarStations() throws DatabaseError {
        try {
            ...
            // Workaround for #56: bdb doesn't seem to handle diacritics
            // properly,
            ...
            // List<String> result = new ArrayList<String>();
            while (r.next()) {
                String identifier = r.getString("wmo") + " "
                        + r.getString("plc");
     ...
     *             In case when file catalog is not available
     */
    public List<BltFile> load(BltQueryParameter param) throws DatabaseError {
        try {
            ExpressionFactory xpr = new ExpressionFactory();
            FileQuery q = new FileQuery();
            Expression ex = getExpression(param);
    ...
    private Expression getExpression(BltQueryParameter param) {
        if (validateParms(param.getRadar(), param.getFileObject(),
            ...
            if (validate(param.getRadar())) {
                String wmoNumber = param.getRadar().substring(0,
                        param.getRadar().indexOf(" "));
    ...
    private boolean validateParms(String radarStation, String fileObject,
        ...
        if (validate(radarStation)
                || validate(fileObject)
                || validate(startDate)
                || (validate(startHour) && validate(startMinute) && validate(startSecond))
                || validate(endDate)
                || (validate(endHour) && validate(endMinute) && validate(endSecond))) {
            result = true;
        }
        return result;
    }
    ...
    /**
     * @param coreFilterManager
     *            the coreFilterManager to set
     */
    @Autowired
