...
import eu.baltrad.dex.net.util.FrameDispatcherServlet;
...
//------------------------------------------------------------------------------------------ Methods    
    ...
    public void testBasicAuthFail() throws IOException {
        ...
        me.addPart( BF_USER_NAME, new StringBody( MessageDigestUtil.createHash( "MD5", 16, "InvalidUser" ) ) );
        me.addPart( BF_PASSWORD, new StringBody( MessageDigestUtil.createHash( "MD5", 16, "bullshit" ) ) );
    ...
    public void testBasicAuthSuccess() throws IOException {
        HttpPost post = new HttpPost( context + SERVLET_PATH );
        MultipartEntity me = new MultipartEntity();
        me.addPart( BF_USER_NAME, new StringBody( MessageDigestUtil.createHash( "MD5", 16, "TestUser" ) ) );
        me.addPart( BF_PASSWORD, new StringBody( MessageDigestUtil.createHash( "MD5", 16, "s3cret" ) ) );
        me.addPart( BF_REQUEST_TYPE, new StringBody( BF_POST_MESSAGE ) );
        me.addPart( BF_MESSAGE_FIELD, new StringBody( "How is it going?" ) );
        post.setEntity( me );
        HttpResponse response = client.execute( post );
